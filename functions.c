...
void compute_shamir_Rq(fmpz_poly_t element, fmpz_poly_t shares[], int players[],
	...
	//Compute the numerator and denominator of the Lagrange coefficients
	int coeffs[2][t+1];
	for(int i=0; i<t+1; ++i) {
		coeffs[0][i]=1;
		coeffs[1][i]=1;
		for(int j=0; j<t+1; ++j) {
			if(j!=i) {
				coeffs[0][i]=coeffs[0][i]*players[j];
				coeffs[1][i]=coeffs[1][i]*(players[j]-players[i]);
			}
		}
	}

	//Compute the addition element and the denominator in the big division aux2
	...
	mpz_set_ui(aux2,1);
	for(int i=0; i<t+1; ++i) {
		mpz_mul_si(aux2,aux2,coeffs[1][i]);
		fmpz_poly_init(aux);
		fmpz_poly_scalar_mul_si(aux,shares[players[i]-1],coeffs[0][i]);
		for(int j=0; j<t+1; ++j) {
			if(j!=i) fmpz_poly_scalar_mul_si(aux,aux,coeffs[1][j]);
		}
		fmpz_poly_add(element,element,aux);
		fmpz_poly_clear(aux);
	}

	//Compute the division
	fmpz_poly_scalar_fdiv_mpz(element,element,aux2);
...
void compute_shamir_Zq(fmpz_t element, fmpz_t shares[], int players[],
	...
	for(int i=0; i<t+1; ++i) {
		mpz_mul_si(aux2,aux2,coeffs[1][i]);
		fmpz_init(aux);
		fmpz_mul_si(aux,shares[players[i]-1],coeffs[0][i]);
		for(int j=0; j<t+1; ++j) {
			if(j!=i) fmpz_mul_si(aux,aux,coeffs[1][j]);
		}
		fmpz_add(element,element,aux);
		fmpz_clear(aux);
	}
	...
	fmpz_init(aux3);
	fmpz_set_mpz(aux3,aux2);
	fmpz_fdiv_q(element,element,aux3);
	//fmpz_p_mod(element,n,element,q);
	mpz_clear(aux2);
	fmpz_clear(aux3);
}
...
*Name: PRSS_share
*
*Decryption:  Computes the PRSS share of an authorized
*             subset of players recursively
*
*Arguments:   int state[]: Vector of integers, signalling
*                          the "forbidden" players already chosen, in particular
*													 the players decrypting smaller than player
*             int indexes[]: Vector of integers, signalling players allowed
*														 to be chosen, in particular all players except those
*														 decrypting smaller or equal to player
*             int size: Integer signalling how many players are left to pick,
*												in particular t-length_state
*             fmpz_poly_t result[]: Polynomial in Rq storing the PRSS share
*																		up to that moment
*             int **key_map: Dictionary storing the keys related to the vector of
*                            "forbidden" players
*							fmpz_t KH[]: Array with the keys KH
*							int keys: number of keys
*             int length_state: Length of state vector
*             int length_indexes: Length of indexes vector
*							fmpz_poly_t lambda: argument of the PRF
*							mpz_t inter: interval of the range of the PRF
*							int n: security parameter
***********************************************************/
void PRSS_share(int state[], int indexes[], int size, fmpz_poly_t result,
	 							int **key_map,fmpz_t KH[],int keys,int length_state,
								int length_indexes,fmpz_poly_t lambda,mpz_t inter,int n) {
  // First we give the condition to end the recursion
  if(size==0){
    // We need to sort state since the keys in the dictionary are sorted
		int* state_new = malloc(length_state * sizeof(int));
		for(int i=0; i<length_state; ++i) state_new[i]=state[i];
    qsort(state_new,length_state,sizeof(int),cmpfunc);
    int r = find_keys(key_map,state_new,length_state,0,keys-1);
    fmpz_poly_t aux;
		fmpz_poly_init(aux);
  ...
  // Then the general case
    ...
    int a=indexes[length_indexes-1];
    if(length_indexes>size) {
      PRSS_share(state,indexes,size,result,key_map,KH,keys,length_state,length_indexes-1,lambda,inter,n);
    }
		state[length_state]=a;
    // We need to sort state since the keys in the dictionary are sorted
    PRSS_share(state,indexes,size-1,result,key_map,KH,keys,length_state+1,length_indexes-1,lambda,inter,n);
  }
  return;
}
